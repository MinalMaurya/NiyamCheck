    ...
    if request.url.path.startswith("/api/"):
        return JSONResponse(
            ...
            content={
                ...
                "details": (
                    f"Method {request.method} on '{request.url.path}' "
                    "is not a registered route. Verify the backend route configuration."
                ),
            },
        )
...
async def unhandled_exception_handler(request: Request, exc: Exception):
    ...
    logger.error(
        f"[UnhandledException] Exception: {type(exc).__name__}: {str(exc)}\n{tb}"
    )
...
async def root():
    return {
        ...
        "milestones": (
            "Milestones 1-7: OCR, Rule Engine, Evidence Mapping, "
            "Aggregation, Legal RAG, PWA & Field Hardening"
        ),
        "endpoints": {
